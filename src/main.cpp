...
#define BEEPER_CHANNEL 6
#define RELAY_CHANNEL 7

// Configuration
// Map input CRSF channels (1-based, up to 16 for CRSF, 12 for ELRS) to outputs
// 1-8 use a negative number to invert the signal (i.e. +100% becomes -100%)
...
constexpr int OUTPUT_FAILSAFE[NUM_OUTPUTS] = {
    ...
    fsaNoPulses, fsaNoPulses, fsaNoPulses, fsaNoPulses // ch5-ch8
};
...
static void servoSetUs(unsigned int servo, int usec) {
  if (usec > 0) {
    ...
    pin_function(OUTPUT_PINS[servo],
                 STM_PIN_DATA(STM_MODE_INPUT, GPIO_PULLDOWN, 0));
  }
...
static void packetChannels() {
  for (unsigned int out = 0; out < NUM_OUTPUTS; ++out) {
    ...
    // BEEPER
    if (chInput == BEEPER_CHANNEL) {
      if (usOutput > 1500)
        digitalWrite(DPIN_BUZ, HIGH);
      else
        digitalWrite(DPIN_BUZ, LOW);
    }
    if (chInput == RELAY_CHANNEL) {
      if (usOutput > 1500)
        digitalWrite(DPIN_REL, HIGH ^ REL_INVERTED);
      else
        digitalWrite(DPIN_REL, LOW ^ REL_INVERTED);
    }
  }
...
static void checkVbatt() {
  ...
  g_State.vbatValue =
      330U * adc * (VBAT_R1 + VBAT_R2) / VBAT_R2 / ((1 << 12) - 1);
  ...
  // Serial.print(" "); Serial.print(g_State.vbatValue, DEC);
  // Serial.println("V");
}
...
static void setupGpio() {
  ...
  pinMode(DPIN_BUZ, OUTPUT);
  pinMode(DPIN_REL, OUTPUT);
  digitalWrite(DPIN_LED, LOW ^ LED_INVERTED);
  digitalWrite(DPIN_BUZ, LOW);
  digitalWrite(DPIN_REL, LOW ^ REL_INVERTED);
